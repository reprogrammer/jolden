...
/**
 * A class that represents a node in a binary tree. Each node represents a city in the TSP
 * benchmark.
 **/
public final class Tree {
  ...
  private Tree right;
  /**
   * The next pointer in a linked list of nodes in the subtree. The list contains the order of the
   * cities to visit.
   **/
  private Tree next;
  /**
   * The previous pointer in a linked list of nodes in the subtree. The list contains the order of
   * the cities to visit.
   **/
  private Tree prev;
  ...
  /**
   * Create a list of tree nodes. Requires root to be the tail of the list. Only fills in next
   * field, not prev.
   * 
  ...
  /**
   * Reverse the linked list. Assumes that there is a dummy "prev" node at the beginning.
   **/
  public void reverse() {
    Tree prev = this.prev;
    prev.next = null;
  ...
  /**
   * Print the list of cities to visit from the current node. The list is the result of computing
   * the TSP problem. The list for the root node (city) should contain every other node (city).
   **/
  ...
  /**
   * Builds a 2D tree of n nodes in specified range with dir as primary axis (false for x, true for
   * y)
   * 
   ...
   **/
  public static Tree buildTree(int n, boolean dir, double min_x, double max_x, double min_y,
      double max_y) {
    if (n == 0) return null;
