...
/**
 * A class that represents the quad edge data structure which implements the edge algebra as
 * described in the algorithm.
 * <p>
 * Each edge contains 4 parts, or other edges. Any edge in the group may be accessed using a series
 * of rotate and flip operations. The 0th edge is the canonical representative of the group.
 * <p>
 * The quad edge class does not contain separate information for vertice or faces; a vertex is
 * implicitly defined as a ring of edges (the ring is created using the next field).
 **/
public class Edge {
  /**
  ...
  /**
   * Return the symmetric of the edge. The symmetric is the same edge with the opposite direction.
   * 
  ...
  /**
   * Return the rotated version of the edge. The rotated version is a 90 degree counterclockwise
   * turn.
   * 
  ...
  /**
   * Return the inverse rotated version of the edge. The inverse is a 90 degree clockwise turn.
   * 
  ...
  public static EdgePair doMerge(Edge ldo, Edge ldi, Edge rdi, Edge rdo) {
    ...
    if (t1 == rdo.orig()) rdo = basel;
    if (t2 == ldo.orig()) ldo = basel.symmetric();
  ...
  /**
   * Print the voronoi diagram and its dual, the delaunay triangle for the diagram.
   **/
