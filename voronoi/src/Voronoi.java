...
/**
 * A Java implementation of the <tt>voronoi</tt> Olden benchmark. Voronoi generates a random set of
 * points and computes a Voronoi diagram for the points.
 * <p>
 * <cite> L. Guibas and J. Stolfi. "General Subdivisions and Voronoi Diagrams" ACM Trans. on
 * Graphics 4(2):74-123, 1985. </cite>
 * <p>
 * The Java version of voronoi (slightly) differs from the C version in several ways. The C version
 * allocates an array of 4 edges and uses pointer addition to implement quick rotate operations. The
 * Java version does not use pointer addition to implement these operations.
 **/
public class Voronoi {
  ...
  /**
   * The main routine which creates the points and then performs the delaunay triagulation.
   * 
  ...
  public static void main(String args[]) {
    ...
    if (printMsgs) System.out.println("Getting " + points + " points");
    ...
    if (printMsgs) System.out.println("Doing voronoi on " + points + " nodes");

    long start1 = System.currentTimeMillis();
    Edge edge = point.buildDelaunayTriangulation(extra);
    long end1 = System.currentTimeMillis();

    if (printResults) edge.outputVoronoiDiagram();
    ...
    while (i < args.length && args[i].startsWith("-")) {
      ...
      if (arg.equals("-n")) {
        if (i < args.length) {
          ...
          throw new RuntimeException("-n requires the number of points");
