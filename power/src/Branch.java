/**
 * A class that represents a branch node in the power pricing architecture. A branch node is another
 * type of intermediate node that represents a split in the electrical power path. The branch nodes
 * hang from the lateral nodes. <b> Each branch node contains a direct link to a set of customers.
 **/
public final class Branch {
  ...
  /**
   * Create all the branch nodes for a single lateral node. Also, create the customers supported by
   * this branch node
   * 
  ...
  public Branch(int num, int nleaves) {
    ...
    if (num <= 1) {
      if (num <= 0) throw new RuntimeException("Branch constructor with zero num");
      nextBranch = null;
    } else {
