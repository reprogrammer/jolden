/**
 * A class that represents a lateral node in the power pricing problem. The lateral nodes is an
 * intermediate node that represents a switch, tappoints, or transformer. It hangs from the root
 * node (the power substation).
 * <p>
...
public final class Lateral {
  ...
  public Lateral(int num, int nbranches, int nleaves) {
    ...
    if (num <= 1) {
      if (num <= 0) throw new RuntimeException("Lateral constructor with zero num");
      next_lateral = null;
    } else {
