...
public final class Demand {
  ...
  /**
   * Create an object that represents power demand and initialize the power demans values.
   * 
  ...
  /**
   * Calculate the power demand given the prices. The pricing problem sets the price for each
   * customer's power consumption so that the economic efficiency of the whole community is
   * maximied.
   * 
  ...
  public void optimizeNode(double pi_R, double pi_I) {
    ...
    do {
      ...
      if (total > 0) {
        ...
        if (magnitude > max_dist) magnitude = max_dist;
      }
      ...
      findGradientH(grad_h);
        ...
        || g > G_EPSILON
        || (Math.abs(g) > G_EPSILON && Math.abs(grad_f[0] * grad_h[1] - grad_f[1] * grad_h[0]) > F_EPSILON));
  }
