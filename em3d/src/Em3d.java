 ...
 * propagation of electromagnetic waves through objects in 3 dimensions. It is a simple computation
 * on an irregular bipartite graph containing nodes representing electric and magnetic field values.
 * 
 * <p>
 * <cite> D. Culler, A. Dusseau, S. Goldstein, A. Krishnamurthy, S. Lumetta, T. von Eicken and K.
 * Yelick. "Parallel Programming in Split-C". Supercomputing 1993, pages 262-273. </cite>
 **/
public class Em3d {
  /**
  ...
  /**
   * The main roitine that creates the irregular, linked data structure that represents the electric
   * and magnetic fields and propagates the waves through the graph.
   * 
  ...
  public static final void main(String args[]) {
    ...
    if (printMsgs) System.out.println("Initializing em3d random graph...");
    ...
    if (printMsgs)
      System.out.println("Propagating field values for " + numIter + " iteration(s)...");
    ...
    if (printResult) System.out.println(graph);
    ...
    while (i < args.length && args[i].startsWith("-")) {
      ...
      if (arg.equals("-n")) {
        if (i < args.length) {
          ...
          throw new Error("-n requires the number of nodes");
      } else if (arg.equals("-d")) {
        if (i < args.length) {
          numDegree = new Integer(args[i++]).intValue();
        } else
          throw new Error("-d requires the out degree");
      } else if (arg.equals("-i")) {
        if (i < args.length) {
          numIter = new Integer(args[i++]).intValue();
        } else
          throw new Error("-i requires the number of iterations");
